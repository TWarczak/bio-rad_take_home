...
First 4 rows and 2 columns of `count_tbl_filt`.
```{r}
...
Now we can group_by, filter, ... `count_tbl_filt_2` by reference genome, chromosome, or locus. The `drops_with_gene` variable was created earlier when filtering out genes present in < 1% of droplets. 
...
Create a summary table to tally unique genes expressed in each droplet. This is different than total counts. If a gene in the `count_tbl` is >= 1, `count_tbl_tally` will count it as 1. 
```{r }
...
Add a plot here of log10(hg19 Counts) vs log10(mm10 Counts), no grouping. Just to show its
easy to identify Mouse + Human doublets. Count those and use that number to find the
number of expected Human + Human and Mouse + Doublets.
...
`expect_quad_2_multi_count` is the expected n doublets in quadrant II. Droplets containing 2+ human OR 2+ mouse cells can be identified by having lower ratio of unique genes to total read counts in the hg19 or mm10 reference genome, respectively. Droplets containing 1+ human AND 1+ mouse cells will have typical ratios of unique genes to total read counts. 
Find the value for nth lowest `unique_count_ratio.` Droplets with values <= to the nth lowest `unique_count_ratio` will be classified as multiplets. Remember to filter counts for quadrant II and IV. 
```{r}
...
Droplets with 2+ Humand OR 2+ Mouse cells typically have higher read counts per droplet for that reference genome. Droplets with 1+ Humand AND 1+ Mouse cells have typical read counts for each reference genome. View histogram of hg19 and mm10 reads on separate plots. 
```{r}
count_tbl_summary %>% 
  filter(total_hg19_counts > 800) %>% 
...
Droplets with 2+ Humand OR 2+ Mouse cells typically have the lowest unique gene count to total read count ratios for that reference genome. Droplets with 1+ Humand AND 1+ Mouse cells have typical ratios for each reference genome. 
...
Another view of how unique gene count to total gene count ratios and total read counts are good indicators of multiplets. Fit a linear model on `total_hg19_counts` for low count droplets, and see how high count droplets deviate from that model. 
```{r}
count_tbl_summary %>% 
...
Save `count_tbl_summary` for further exploratory data analysis. 
```{r}
data.table::fwrite(count_tbl_summary, here("data", "count_tbl_summary.csv"))
```
